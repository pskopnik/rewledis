...
// call panics.
//
// The purpose of At is to allow SlotDeque to serve as a more general purpose
...
// back-to-front. Having SlotDeque provide Rotate() avoids resizing that could
...
func (q *SlotDeque) prev(i int) int {
	return (i - 1) & (len(q.buf) - 1) // bitwise modulus
}

// next returns the next buffer position wrapping around buffer.
func (q *SlotDeque) next(i int) int {
	return (i + 1) & (len(q.buf) - 1) // bitwise modulus
}

// growIfFull resizes up if the buffer is full.
func (q *SlotDeque) growIfFull() {
	if len(q.buf) == 0 {
		q.buf = make([]Slot, minCapacity)
		return
	}
...
func (q *SlotDeque) shrinkIfExcess() {
...
// only a quarter full.
func (q *SlotDeque) resize() {
	newBuf := make([]Slot, q.count<<1)
	if q.tail > q.head {
